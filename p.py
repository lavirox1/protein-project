...
def write_ca_pdb(seq, ca_coords, out_path, proxy_plddt=None):
    with open(out_path, "w") as f:
        serial = 1
        for i,(aa,xyz) in enumerate(zip(seq, ca_coords.tolist()), start=1):
            x,y,z = xyz
            b = 0.0 if proxy_plddt is None else float(proxy_plddt)   # store proxy in B-factor
            f.write(f"ATOM  {serial:5d}  CA  {aa:>3s} A{i:4d}    {x:8.3f}{y:8.3f}{z:8.3f}  1.00 {b:6.2f}           C\n")
            serial += 1
        f.write("END\n")
...
def render_pymol_png(pdb_path: str, out_png: str, pymol_exe: Optional[str] = None):
    pml = f"""
    reinitialize
    load {pdb_path}, prot
    hide everything
    ...
    quit
    """
    script = Path("render_tmp.pml"); script.write_text(pml)
    try:
        cmds = []
        if pymol_exe:
            cmds.append([pymol_exe, "-cq", str(script)])
        cmds.append([sys.executable, "-m", "pymol", "-cq", str(script)])
        cmds.append(["pymol.exe", "-cq", str(script)])
        cmds.append(["pymol", "-cq", str(script)])
        for cmd in cmds:
            try:
                subprocess.run(cmd, check=True)
                print(f"[PyMOL] Saved {out_png}")
                return
            except FileNotFoundError:
                continue
            except subprocess.CalledProcessError as e:
                # keep trying the next command
                last_err = e
        print("[WARN] PyMOL not found/failed; using matplotlib fallback.")
        render_matplotlib_ca(pdb_path, out_png)
    finally:
        if script.exists(): 
...
def main():
    ...
    if args.predict:
        ...
        try:
            render_pymol_png(out_pdb, "generated_predicted.png", pymol_exe=r'C:\Users\Anvay Borade\AppData\Local\Schrodinger\PyMOL2\PyMOLWin.exe')
